...
const ServiceApplicationCard: React.FC<ServiceApplicationCardProps> = ({
  ...
  return (
    <div className={`relative flex items-center w-full ${isLeftAligned ? "flex-row" : "flex-row-reverse"}`}>
      ...
      {/* Circle Icon */}
      <div className="relative w-28 h-28 flex items-center justify-center bg-blue-500 rounded-full shadow-lg border-4 border-blue-300 z-10">
      ...
      <div className={`absolute ${isLeftAligned ? "left-28" : "right-28"} top-1/2 transform -translate-y-1/2 h-4 bg-blue-500`} style={{ width: 'calc(100% - 7rem)' }}></div>
      </div>
